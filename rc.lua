...
local function table_range(from, to, step)
  step = step or 1
  local tab = {}
...
local themedir = '/usr/share/awesome/themes'
local theme = 'gentoo'
...
local myawesomemenu = {
   ...
   {"edit config", table.concat({editor_cmd, awesome.conffile}, ' ')},
...
ots:buttons(awful.util.table.join(awful.button({ }, 1, function()
  naughty.notify{
    title = 'OTP passwords:',
    text = trim(awful.util.pread('onenightstand')),
    timeout = 10
  }
end)))
...
local globalkeys = awful.util.table.join(
  ...
  -- Prompt
  awful.key({ modkey },            "r",     function () awful.util.spawn(table.concat({
...
for tag, i in ipairs(tagsymbols) do
  globalkeys = awful.util.table.join(globalkeys,
    -- View Tag
    awful.key({ modkey }, "#" .. i, function ()
      local screen = mouse.screen
      local tag = awful.tag.gettags(screen)[tag]
      if tag then
    ...
    awful.key({ modkey, "Shift" }, "#" .. i, function ()
      if client.focus then
        local tag = awful.tag.gettags(client.focus.screen)[tag]
        if tag then
...
client.connect_signal("manage", function (c, startup)
  ...
  if titlebars and (c.type == "normal" or c.type == "dialog") then
