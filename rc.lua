...
for i, symbol in ipairs(tagsymbols) do
  local name = tagnames[i]
  globalkeys = awful.util.table.join(globalkeys,
  -- View tag only.
  awful.key({ modkey }, "#" .. symbol,
  function ()
  ...
  -- Toggle tag display.
  awful.key({ modkey, "Control" }, "#" .. symbol,
  function ()
    local screen = awful.screen.focused()
    local tag = screen.tags[i]
    if tag then
  ...
  awful.key({ modkey, "Control", "Shift" }, "#" .. symbol,
  function ()
    if client.focus then
      local tag = client.focus.screen.tags[i]
      if tag then
